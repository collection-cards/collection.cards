...
import {SPINE_SIZES, spineSizes} from '../../sizes'
...
export async function GET(
  ...
  const normalized = size.toLowerCase() as (typeof SPINE_SIZES)[number]
  if (!SPINE_SIZES.includes(normalized))
    return NextResponse.json(
      {error: `Invalid size. Must be ${SPINE_SIZES.join(', ')}`},
      {status: 400}
    )
  ...
  const collectionIcon = setData.collection?.[0]?.icon

  const {width, height} = spineSizes[normalized]
  const filename = `${setData.path}-binder-spine-${normalized}-collection-cards.png`

  const sourceWidth = logo?.width ?? 1
  const sourceHeight = logo?.height ?? 1
  const targetLogoHeight = (width / 100) * logoWidthPercentage
  const targetLogoWidth = (sourceWidth * targetLogoHeight) / sourceHeight
  const logoUrl = new URL(
    `${process.env.PUBLIC_SITE_URL}/media${logo?.src}`,
    request.url
  ).toString()

  const sourceCollectionWidth = collectionIcon.width ?? 1
  const sourceCollectionHeight = collectionIcon.height ?? 1
  const targetCollectionLogoWidth = (width / 100) * logoWidthPercentage
  const targetCollectionLogoHeight =
    (sourceCollectionHeight * targetCollectionLogoWidth) / sourceCollectionWidth
  const collectionLogoUrl = new URL(
    `${process.env.PUBLIC_SITE_URL}/media${collectionIcon?.src}`,
    request.url
  ).toString()
  ...
  return new ImageResponse(
    ...
    >
      ...
      {setData.symbols?.map((symbol, index) => {
        const symbolSourceWidth = symbol.width ?? 1
        const symbolSourceHeight = symbol.height ?? 1
        const symbolTargetHeight = (width / 100) * (logoWidthPercentage / 1.5)
        const symbolTargetWidth =
          (symbolSourceWidth * symbolTargetHeight) / symbolSourceHeight
        const symbolUrl = new URL(
          `${process.env.PUBLIC_SITE_URL}/media${symbol.src}`,
          request.url
        ).toString()
        ...
        return (
          <img
            ...
            src={symbolUrl}
            alt={setData.title}
            width={symbolTargetWidth}
            height={symbolTargetHeight}
            style={{
              width: `${symbolTargetWidth}px`,
              height: `${symbolTargetHeight}px`,
              position: 'absolute',
              top: `${(width - targetCollectionLogoWidth) / 2}px`
            }}
          />
        )
      })}
      ...
      <img
        ...
        alt={logo.title || setData.title}
        width={targetCollectionLogoWidth}
        height={targetCollectionLogoHeight}
        style={{
          width: `${targetCollectionLogoWidth}px`,
          height: `${targetCollectionLogoHeight}px`,
          position: 'absolute',
          bottom: `${(width - targetCollectionLogoWidth) / 2}px`
        }}
      />
    </div>,
    {
